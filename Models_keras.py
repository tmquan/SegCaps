...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1e-4, block_prefix=None):
    ...
    with K.name_scope('ConvBlock'):
        ...
        x = KL.BatchNormalization(axis=concat_axis, epsilon=1.1e-5, name=name_or_none(block_prefix, '_bn'))(ip)
        ...
        x = KL.Conv2D(nb_filter, (3, 3), kernel_initializer='he_normal', padding='same', use_bias=False,
...
def __transition_block(ip, nb_filter, compression=1.0, weight_decay=1e-4, block_prefix=None, transition_pooling='max'):
    ...
    with K.name_scope('Transition'):
        ...
        x = KL.BatchNormalization(axis=concat_axis, epsilon=1.1e-5, name=name_or_none(block_prefix, '_bn'))(ip)
        x = KL.Activation('relu')(x)
        x = KL.Conv2D(int(nb_filter * compression), (1, 1), kernel_initializer='he_normal', padding='same',
                   use_bias=False, kernel_regularizer=l2(weight_decay), name=name_or_none(block_prefix, '_conv2D'))(x)
        if transition_pooling == 'avg':
            x = KL.AveragePooling2D((2, 2), strides=(2, 2))(x)
...
def __transition_up_block(ip, nb_filters, type='deconv', weight_decay=1E-4, block_prefix=None):
    ...
    with K.name_scope('TransitionUp'):
        ...
        if type == 'upsampling':
            x = KL.UpSampling2D(name=name_or_none(block_prefix, '_upsampling'))(ip)
        elif type == 'subpixel':
            x = KL.Conv2D(nb_filters, (3, 3), activation='relu', padding='same', kernel_regularizer=l2(weight_decay),
            ...
            x = KL.Conv2DTranspose(nb_filters, (3, 3), activation='relu', padding='same', strides=(2, 2),
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
    ...
    with K.name_scope('DenseNet'):
        ...
        x = KL.Conv2D(nb_filter, initial_kernel, kernel_initializer='he_normal', padding='same', name='initial_conv2D',
                   strides=initial_strides, use_bias=False, kernel_regularizer=l2(weight_decay))(img_input)

        if subsample_initial_block:
            x = KL.BatchNormalization(axis=concat_axis, epsilon=1.1e-5, name='initial_bn')(x)
            x = KL.Activation('relu')(x)
        ...
        x = KL.BatchNormalization(axis=concat_axis, epsilon=1.1e-5, name='final_bn')(x)
        x = KL.Activation('relu')(x)
        ...
        if include_top:
            ...
            x = KL.Dense(nb_classes, activation=activation)(x)
        else:
            if pooling == 'avg':
                x = KL.GlobalAveragePooling2D()(x)
            elif pooling == 'max':
...
@memoized
def create_fcn_dense_net(img_input, nb_classes=1, include_top=True, nb_dense_block=5, growth_rate=12,
                           reduction=0.0, dropout_rate=None, weight_decay=1e-4,
                           nb_layers_per_block=4, nb_upsampling_conv=128, upsampling_type='deconv',
                           init_conv_filters=48, input_shape=None, activation='softmax',
                           early_transition=False, transition_pooling='max', initial_kernel_size=(3, 3)):
    ...
    # Initial convolution
    x = KL.Conv2D(init_conv_filters, initial_kernel_size, kernel_initializer='he_normal', padding='same', name='initial_conv2D',
               use_bias=False, kernel_regularizer=l2(weight_decay))(img_input)
    x = KL.BatchNormalization(axis=concat_axis, epsilon=1.1e-5, name='initial_bn')(x)
    x = KL.Activation('relu')(x)
    ...
    if early_transition:
        x_up = __transition_up_block(x_up, nb_filters=nb_filter, type=upsampling_type, weight_decay=weight_decay,
                                     block_prefix='tr_up_early')
    if include_top:
        x = KL.Conv2D(nb_classes, (1, 1), activation='linear', padding='same', use_bias=False)(x_up)
        ...
        x = KL.Reshape((row * col, nb_classes))(x)
